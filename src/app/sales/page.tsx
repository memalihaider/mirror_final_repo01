...
// export default function SalesReportPage() {
//   const [bookings, setBookings] = useState<BookingDocLite[]>([]);
//   const [filteredBookings, setFilteredBookings] = useState<BookingDocLite[]>([]);
...
//           paymentMethod: data.paymentMethod || '',
//           cardAuthCode: data.cardAuthCode || '',
//           // ⬇️ Pull discount and tip from booking-level fields
...
//       filtered = filtered.filter((b) => b.branch === selectedBranch);
//     }
//     if (dateFilter) {
//       filtered = filtered.filter(
//         (b) => format(b.bookingDate, 'yyyy-MM-dd') === dateFilter
//       );
//     }
//     setFilteredBookings(filtered);
//   }, [bookings, selectedBranch, dateFilter]);
...
//       (sum, b) =>
//         sum +
//         b.services.reduce((s, sv) => s + sv.price, 0) -
//         (b.discount || 0) +
//         (b.tipAmount || 0),
//       0
//     );
//     const totalTips = filteredBookings.reduce((s, b) => s + (b.tipAmount || 0), 0);
//     const totalDiscounts = filteredBookings.reduce((s, b) => s + (b.discount || 0), 0);
//     return { totalSales, totalTips, totalDiscounts };
//   }, [filteredBookings]);
...
//       Tip: b.tipAmount || 0,
//       Total: b.services.reduce((s, sv) => s + sv.price, 0) - (b.discount || 0) + (b.tipAmount || 0),
//     }));
...
//       b.branch,
//       b.paymentMethod,
//       b.discount?.toFixed(2) || '0.00',
//       b.tipAmount?.toFixed(2) || '0.00',
//       (
//         b.services.reduce((s, sv) => s + sv.price, 0) -
//         (b.discount || 0) +
//         (b.tipAmount || 0)
...
//       head: [['Customer', 'Services', 'Date', 'Branch', 'Payment', 'Discount', 'Tip', 'Total']],
//       body: tableRows,
//     });
//     doc.save(`SalesReport_${Date.now()}.pdf`);
//   };
...
//           {[...new Set(bookings.map((b) => b.branch))].map((branch) => (
...
//                 <td className="px-4 py-2">Rs {b.discount?.toFixed(2) || '0.00'}</td>
//                 <td className="px-4 py-2">Rs {b.tipAmount?.toFixed(2) || '0.00'}</td>
