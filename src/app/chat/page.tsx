...
function ChatComponent() {
  ...
  // Refs to hold latest state for use inside realtime callbacks (avoid stale closures)
  ...
  useEffect(() => {
    // audio handled globally by GlobalChatNotifier
  }, []);
  ...
  // sound handled by GlobalChatNotifier
      ...
      try {
        ...
        unsubscribe = onSnapshot(chatRoomsQuery, (snapshot) => {
          try {
            ...
            // Check for new messages in chat rooms (use ref to avoid stale closures)
            const prevRooms = chatRoomsRef.current;
            if (!isLoading && prevRooms.length > 0) {
              rooms.forEach(room => {
                const previousRoom = prevRooms.find(r => r.id === room.id);
                if (previousRoom && room.lastMessage !== previousRoom.lastMessage &&
                  ...
                  const curSelected = selectedChatRoomRef.current;
                  if (!curSelected || curSelected.id !== room.id) {
  ...
  // keep selectedChatRoom ref in sync to avoid stale closures in callbacks
  useEffect(() => {
      ...
      try {
        ...
        unsubscribe = onSnapshot(messagesQuery, (snapshot) => {
          try {
            ...
            // Use messagesRef to compare previous messages to avoid stale closure
            const prevMsgs = messagesRef.current;
            if (prevMsgs.length > 0 && msgs.length > prevMsgs.length) {
              const newMessages = msgs.slice(prevMsgs.length);
              // sound handled by GlobalChatNotifier

              newMessages.forEach(msg => {
                // If incoming message from user and we're not in the same chat room, show visual/browser notification
                const curSelected = selectedChatRoomRef.current;
                if (msg.senderType === 'user' && (!curSelected || curSelected.id !== msg.chatRoomId)) {
                  showNotification(msg.chatRoomId, msg.senderName, msg.content);
                }
              });
            }
