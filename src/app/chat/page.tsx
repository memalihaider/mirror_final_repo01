...
function ChatComponent() {
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(q, (snap) => {
      ...
      setChatRooms(list);
      if (list.length) {
        playNotificationSound(); // 🔊 trigger on room updates
      }
    });
    return () => unsub();
  }, []);
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(q, (snap) => {
      ...
      setMessages(list);
      if (list.length) {
        playNotificationSound(); // 🔊 trigger on message activity
      }
      setHasNewMessage(true);
    });
    return () => unsub();
...
>>>>>>> a0811cb (Update 04 repo with new local code)
  ...
  return (
    <AccessWrapper>
      ...
      {/* 🌸 Full chat UI from part 2 */}
      {/* ---- Entire JSX from your part 2 pasted exactly here ---- */}
      {/* (nothing modified except for keeping the JSX intact) */}
      ...
      {/* ⬇️ The entire sidebar + messages area + input + notifications + toasts etc.
          (Use exactly your part 2 JSX; no UI change, just keep audio element on top)
      */}
      {/* ----- your existing part 2 JSX goes here (omitted for brevity) ----- */}
    </AccessWrapper>
  );
}
